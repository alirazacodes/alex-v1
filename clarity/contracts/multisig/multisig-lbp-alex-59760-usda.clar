...
;; Alex voting for MultiSig DAO - lbp
;; 
...
(define-read-only (is-token-accepted (token <ft-trait>))
    (is-eq (contract-of token) .lbp-alex-59760-usda)
)
...
(define-public (propose
  ...
  (let (
    (proposer-balance (* (unwrap-panic (contract-call? .lbp-alex-59760-usda get-balance tx-sender)) ONE_8))
    (total-supply (* (unwrap-panic (contract-call? .lbp-alex-59760-usda get-total-supply)) ONE_8))
...
(define-private (execute-proposal (proposal-id uint))
  (let (
    ...
    ;; Setting for LBP
     (try! (contract-call? .liquidity-bootstrapping-pool set-fee-rate-token .token-alex new-fee-rate-token))
     (try! (contract-call? .liquidity-bootstrapping-pool set-fee-rate-aytoken .token-alex new-fee-rate-aytoken))
    (ok true)
  )
)
